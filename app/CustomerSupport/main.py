...
ORDERS = {
    "ORD-12345": {"status": "Shipped", "items": ["Wireless Headphones", "USB-C Hub"], "total": 134.98},
...
def get_order(order_id: str) -> str:
    """Simulate fetching order details by order ID.

    Args:
        order_id: Order ID (e.g., 'ORD-12345')

    Returns:
        Simulated order details including status and items
    """
    
    order = ORDERS.get(order_id.upper())
    if order:
        items_str = ", ".join(order["items"])
        return f"Order {order_id}: Status: {order['status']}, Items: {items_str}, Total: ${order['total']}"
...
def get_or_create_agent(session_id, user_id, auth_header):
    ...
    tools = [get_return_policy, get_product_info, get_order]
